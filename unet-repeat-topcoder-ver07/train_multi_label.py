import os
os.environ['CUDA_VISIBLE_DEVICES'] = '3,2' #'3,2,1,0'
...
def mask_to_annotation(mask):
    foreground = mask_to_foreground(mask).astype(np.float32)
    cut = mask_to_cut(mask).astype(np.float32)
...
def run_train():
    ...
    out_dir = RESULTS_DIR + '/unet-se-resnext101-multi-9'
    initial_checkpoint = \
        RESULTS_DIR + '/unet-se-resnext101-multi-9/checkpoint/00007400_model.pth'
        #
    ...
    optimizer = optim.SGD(filter(lambda p: p.requires_grad, net.parameters()),
                          lr=0.002/iter_accum, momentum=0.9, weight_decay=0.0001)
