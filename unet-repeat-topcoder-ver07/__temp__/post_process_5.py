...
if __name__ == '__main__':
    ...
    for i in range(num_instances):
        ...
        # cut_overlay = image.copy()
        # cut_overlay[cut]=[0,255,255]
        # image_show_norm('cut_overlay',cut_overlay,resize=1)
        # image_show('image',image,resize=1)
        # cv2.waitKey(0)

    cut_overlay = image.copy()
    cut_overlay = mask_to_contour_overlay(mask,cut_overlay,[255,0,0])
    cut_overlay[cut]=[0,255,0]

    image_show_norm('cut_overlay',cut_overlay,resize=1)
    image_show_norm('inner',inner,resize=1)
    # cv2.waitKey(0)
    # exit(0)

    # img_copy = np.copy(mask_img)
    foreground = binary.astype(np.float32)
    ...
    marker   = skimage.morphology.label(m>0.5)
    distance = foreground>0.5
    water    = skimage.morphology.watershed(distance, marker, mask=foreground>0.5)  #, watershed_line=True)


    contour_overlay = mask_to_contour_overlay(mask,image,[0,255,0])
    contour_overlay = mask_to_contour_overlay(water,contour_overlay,[0,0,255])
