...
# go through positive and negative dirs
for label_type in ['neg', 'pos']:
    ...
    for fname in os.listdir(dir_name):
        ...
        if fname[-4:] == '.txt':
            # Add the file's content to texts array
            f = open(os.path.join(dir_name, fname), encoding="utf8")
            texts.append(f.read())
            f.close()
            # Add the file's label to labels array
            if label_type == 'neg':
                labels.append(0)
            else:
                labels.append(1)
...
glove_dir = '/Users/bk/desktop/Machine-Learning/'
